...
;;;; Time-stamp: <2022-06-22 20:21:50 marcusp>                           
;;;; ======================================================================
...
(defun general-chord-type-2014 (pitch-class-set pitch-scale-hierarchy consonance-vector)
...
(defun relate-to-key (chord pitch-scale-hierarchy)
  (let* ((root (first chord))
    ...
    (if (null tonic)
        chord
        (list (utils:subtract-mod-n root tonic 12) base extension))))
...
(defun add-root (chord)
  (let* ((base (first chord))
         ...
         (root (car base)))
    (list root 
          (mapcar #'(lambda (x) (utils:subtract-mod-n x root 12)) base) 
          (mapcar #'(lambda (x) (utils:subtract-mod-n x root 12)) extensions))))
...
(defun make-compact (list)
  (flet ((compactness (list n)
           (let ((first (car list))
                 (last (car (last list))))
             (utils:subtract-mod-n last first n))))
